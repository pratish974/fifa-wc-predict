...
export default function LoginPage() {
  ...
  const usernameIsValid = trimmedUsername.length > 0 && namePattern.test(trimmedUsername);
  ...
  const login = async () => {
    ...
    if (!usernameIsValid) {
      setError('Invalid username. Use letters, spaces, hyphens, or underscores only.');
      return;
    }
  ...
  return (
    <div>
      ...
      {isAdmin ? (
        <>
          <label htmlFor="password" style={{ display: 'block', marginBottom: 8, marginTop: 16 }}>
            Admin password
          </label>
          <input
            id="password"
      ...
      <button
        onClick={login}
        disabled={loading || !usernameIsValid || !matchedUser || (isAdmin && !password)}
